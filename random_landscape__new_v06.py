...
for db in range(1):
    ...
    while frag_left==1:
        ...
        rows_frag = arcpy.SearchCursor(shape_to_aleat,fields="CENTROID_X;CENTROID_Y",sort_fields="AR_SHORT")
        ...
        arcpy.Dissolve_management(output_file+'/temp_clip',output_file+'/temp_clip_diss', '#', '#', 'MULTI_PART', 'DISSOLVE_LINES')   
        arcpy.MultipartToSinglepart_management(output_file+'/temp_clip_diss',
                                                                       output_file+'/temp_clip_diss_exp')        
        insert_to( output_file+'/temp_clip_diss_exp', arc_acumula)
        arcpy.CopyFeatures_management(output_file+'/temp_clip',output_file+'/'+shp_intersect)
