...
#mapa original
frags_ori=arcpy.GetParameterAsText(0)
limite_area=arcpy.GetParameterAsText(1)


out_folder_path=limite_area=arcpy.GetParameterAsText(2)
nsimulation=limite_area=arcpy.GetParameterAsText(3)
nsimulation=int(nsimulation)
...
for db in range(nsimulation):
    ...
    while frag_left==1:
        ...
        for row in rows_frag:
            ...
            if first_time==1:
                arcpy.CreateRandomPoints_management(output_file, 'pnt_rnd'+format,limite_area, '', '1', '0 Meters', 'POINT', '0')
            ...
            # se nao, o limite a ser usado para sortear pontos vai ser o limite menos o poligono que foi acresentado na rodada anterior, e isso vai
                ...
                arcpy.Erase_analysis(limite_area, shape_to_aleat, output_file+'/new_limit'+format, '#')
        ...
        if count_intersect_lines>0:
            ...
            insert(out_folder_path+'/'+frags_ori+'_random_'+format_name_db)
