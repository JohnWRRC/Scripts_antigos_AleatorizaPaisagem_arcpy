...
output_file=folder+'/trash11.gdb'

#________________________________________

#mapa original
frags_ori=arcpy.GetParameterAsText(1)
...
env.workspace=r'E:\data_2015\___john\marcos_ufrpe\shp\SAD69_teste\trash11.gdb\\'
out_path = r'E:\data_2015\___john\marcos_ufrpe\shp\SAD69_teste\trash11.gdb\\'
...
while frag_left==1:
    ...
    for row in rows_frag:
        ...
        if first_time==1:
            ...
            arcpy.Erase_analysis(limite, shape_to_aleat, output_file+'/new_limit'+format, '#')
    ...
    #aasd
    arcpy.Erase_analysis(output_file+'/'+shp_to,limite, output_file+'/out_patchs', '#')
    arcpy.Dissolve_management(output_file+'/out_patchs',output_file+'/out_patchs_diss', '#', '#', 'MULTI_PART', 'DISSOLVE_LINES')   
    arcpy.MultipartToSinglepart_management(output_file+'/out_patchs_diss',
                                           output_file+'/out_patchs_diss_exp')    
    
    insert_to(output_file+'/out_patchs_diss_exp',output_file+'/'+shp_intersect)
    
    count_intersect_lines=con_lines(output_file+'/'+shp_intersect)
    arcpy.Clip_analysis(output_file+'/'+shp_to,limite,output_file+'/temp_clip')
    ...
    if count_intersect_lines>0:
        ...
        env.workspace=folder+'/trash11.gdb\\'
        out_path = folder+'/trash11.gdb\\'
        shp_intersect ='shp_intersect'
        geometry_type = "POLYGON"
        template = frags_com_pacth
        has_m = "DISABLED"
